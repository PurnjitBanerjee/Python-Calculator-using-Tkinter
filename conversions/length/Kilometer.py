def km():
    convertion_to = str(input("Enter the unit you want to convert to:"))
    if convertion_to == "inch":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 39370.1, "inch")
    elif convertion_to == "foot":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 3280.84, "foot")
    elif convertion_to == "m":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 1000, "m")
    elif convertion_to == "mm":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 1000000, "mm")
    elif convertion_to == "cm":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 100000, "cm")
    elif convertion_to == "mile":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 0.621371, "mile")
    elif convertion_to == "yards":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 1093.613, "yards")
    elif convertion_to == "micro":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 1000000000, "micro")
    elif convertion_to == "nano":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 1000000000000, "nano")
    elif convertion_to == "nat. miles":
        num1 = float(input("Enter the number of km:"))
        print("Result is", num1 * 0.539957, "nat. miles")
    else:
        print("Invalid input")
